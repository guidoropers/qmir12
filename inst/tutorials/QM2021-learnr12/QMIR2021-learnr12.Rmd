...
Let's take a look at our data `df_learnr12`. In the first step, we want you to run a pooled model using the following variables: `n_RAI`, `length_EU`, `length_world`, `democ`, `gdp_capita`, `population_size`, and `nationalization`.
...
```{r q5, echo = FALSE}
question_text("What is the coefficient of `length_world`.",
...
```{r q7, echo = FALSE}
question_text("What is the coefficient for the duration of World Bank membership? Round the coefficient to two digits after the decimal.",
...
In the next step, we want to calculate predicted values. Remember from the last sessions that predicted values can help us to communicate the results of our analysis. In the following section, the model has already been specified. Please calculate predicted values for membership in the **World Bank**. Compare the predictions for the maximum duration of World Bank membership. In which of the two countries is the predicted value higher?
